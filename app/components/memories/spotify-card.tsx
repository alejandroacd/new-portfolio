"use client";

import { useEffect, useState } from "react";
import { profile } from "@/app/lib/profile";

export function SpotifyCard() {
  const [loaded, setLoaded] = useState(false);
  const [timedOut, setTimedOut] = useState(false);

  useEffect(() => {
    const t = setTimeout(() => {
      if (!loaded) setTimedOut(true);
    }, 5000);
    return () => clearTimeout(t);
  }, [loaded]);

  return (
    <div className="bento-card flex flex-col gap-4 p-6 sm:p-8 lg:col-span-2 lg:row-span-2 sm:col-span-2 min-h-[680px] sm:min-h-[500px]">
      <div className="flex items-center justify-between">
        <p className="section-label">On repeat</p>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-emerald-400">
          <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.42 1.56-.299.421-1.02.599-1.559.3z"/>
        </svg>
      </div>

      <div className="relative flex-1 overflow-hidden rounded-xl bg-zinc-900/60">
        {!loaded && !timedOut && (
          <div className="absolute inset-0 flex flex-col gap-3 p-4 animate-pulse">
            <div className="h-16 w-16 rounded-md bg-zinc-800" />
            <div className="space-y-2">
              <div className="h-3 w-3/4 rounded bg-zinc-800" />
              <div className="h-2 w-1/2 rounded bg-zinc-800" />
            </div>
            <div className="mt-auto h-2 w-full rounded-full bg-zinc-800" />
          </div>
        )}

        {timedOut && !loaded ? (
          <a
            href={profile.spotifyEmbedUrl.replace("/embed/", "/")}
            target="_blank"
            rel="noopener noreferrer"
            className="absolute inset-0 flex items-center justify-center text-sm text-zinc-400 hover:text-white"
          >
            Open in Spotify →
          </a>
        ) : (
          <iframe
            src={profile.spotifyEmbedUrl}
            width="100%"
            height="100%"
            frameBorder="0"
            allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
            loading="lazy"
            onLoad={() => setLoaded(true)}
            className={`absolute  inset-0 h-full w-full transition-opacity duration-500 ${
              loaded ? "opacity-100" : "opacity-0"
            }`}
          />
        )}
      </div>
    </div>
  );
}
